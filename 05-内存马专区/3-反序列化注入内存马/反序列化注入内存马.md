...
在前面关于 Tomcat 内存马的学习中，我么会初步了解了内存马的原理，但也遗留下了一些问题。之前我们的内存马注入是通过上传 JSP 文件进而注入到内存中，在实战中，这样实现的效果和直接上传一个 JSP 马似乎并无不同，此外 JSP 编译器也会编译我们上传的 JSP 文件生成对应的`.java`和`.class`文件造成文件落地，极大概率会触发安全告警。
